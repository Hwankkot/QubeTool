...
CGreenCube* CGreenCube::Create(LPDIRECT3DDEVICE9 pDevice, Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
...
HRESULT CGreenCube::InitCube(Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	...
	/// 정해진 박스 스케일을 받는다.
	float fCubeScale = CGreenCube::CUBESCALE;
...
void CGreenCube::UpdateCollision(void)
{ 
	...
	{ // if
		CGreenCube* pCube = NULL;

		switch(wEdge)
		{ // switch
		case PLANE_TOP:
			pCube = CGreenCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.y = m_pInfo->m_vPos.y + CGreenCube::CUBERADIUS;
			break;
		...
		case PLANE_LEFT:
			pCube = CGreenCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.x = m_pInfo->m_vPos.x - CGreenCube::CUBERADIUS;
			break;
		...
		case PLANE_FRONT:
			pCube = CGreenCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.z = m_pInfo->m_vPos.z - CGreenCube::CUBERADIUS;
			break;
...
void CGreenCube::UpdateMovement(void)
{
...
bool CGreenCube::IsIntersectedtoSelect(D3DXVECTOR3* pPos, WORD* pEdge)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
bool CGreenCube::IsIntersectedtoMakeNavCell(D3DXVECTOR3* pPos)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
void CGreenCube::ReleaseSelect(void)
{
	m_pSphere->ChangeColor(D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
	m_IsSelected = false; 
}

Engine::CTransform* CGreenCube::GetCubeData(void)
{
	return m_pInfo;
}
