...
CCubeWall::CCubeWall(LPDIRECT3DDEVICE9 pDevice)
: CCube(pDevice)
...
CCubeWall::~CCubeWall(void)
{
	Release();
}

CCubeWall* CCubeWall::Create(LPDIRECT3DDEVICE9 pDevice, Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	CCubeWall* pGameObject = new CCubeWall(pDevice);
...
HRESULT CCubeWall::InitCube(Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	...
	/// 정해진 박스 스케일을 받는다.
	float fCubeScale = CCubeWall::CUBESCALE;
...
HRESULT CCubeWall::AddComponent(void)
{
	...
	// Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCEID::STATICMESH_CUBE_WALL);
...
void CCubeWall::UpdateCollision(void)
{ 
	...
	{ // if
		CCubeWall* pCube = NULL;

		switch(wEdge)
		{ // switch
		case PLANE_TOP:
			pCube = CCubeWall::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.y = m_pInfo->m_vPos.y + CCubeWall::CUBERADIUS;
			break;
		...
		case PLANE_LEFT:
			pCube = CCubeWall::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.x = m_pInfo->m_vPos.x - CCubeWall::CUBERADIUS;
			break;
		...
		case PLANE_FRONT:
			pCube = CCubeWall::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.z = m_pInfo->m_vPos.z - CCubeWall::CUBERADIUS;
			break;
...
void CCubeWall::UpdateMovement(void)
{
...
bool CCubeWall::IsIntersectedtoSelect(D3DXVECTOR3* pPos, WORD* pEdge)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
bool CCubeWall::IsIntersectedtoMakeNavCell(D3DXVECTOR3* pPos)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
void CCubeWall::ReleaseSelect(void)
{
	m_pSphere->ChangeColor(D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
	m_IsSelected = false; 
}

Engine::CTransform* CCubeWall::GetCubeData(void)
{
	return m_pInfo;
}

void CCubeWall::Release(void)
{
