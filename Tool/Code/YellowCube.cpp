...
CYellowCube* CYellowCube::Create(LPDIRECT3DDEVICE9 pDevice, Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
...
HRESULT CYellowCube::InitCube(Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	...
	/// 정해진 박스 스케일을 받는다.
	float fCubeScale = CYellowCube::CUBESCALE;
...
void CYellowCube::UpdateCollision(void)
{ 
	...
	{ // if
		CYellowCube* pCube = NULL;

		switch(wEdge)
		{ // switch
		case PLANE_TOP:
			pCube = CYellowCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.y = m_pInfo->m_vPos.y + CYellowCube::CUBERADIUS;
			break;
		...
		case PLANE_LEFT:
			pCube = CYellowCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.x = m_pInfo->m_vPos.x - CYellowCube::CUBERADIUS;
			break;
		...
		case PLANE_FRONT:
			pCube = CYellowCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.z = m_pInfo->m_vPos.z - CYellowCube::CUBERADIUS;
			break;
...
void CYellowCube::UpdateMovement(void)
{
...
bool CYellowCube::IsIntersectedtoSelect(D3DXVECTOR3* pPos, WORD* pEdge)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
bool CYellowCube::IsIntersectedtoMakeNavCell(D3DXVECTOR3* pPos)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
void CYellowCube::ReleaseSelect(void)
{
	m_pSphere->ChangeColor(D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
	m_IsSelected = false; 
}

Engine::CTransform* CYellowCube::GetCubeData(void)
{
	return m_pInfo;
}
