...
CPurpleCube::CPurpleCube(LPDIRECT3DDEVICE9 pDevice)
: CCube(pDevice)
...
CPurpleCube::~CPurpleCube(void)
{
	Release();
}

CPurpleCube* CPurpleCube::Create(LPDIRECT3DDEVICE9 pDevice, Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	CPurpleCube* pGameObject = new CPurpleCube(pDevice);
...
HRESULT CPurpleCube::InitCube(Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	...
	/// 정해진 박스 스케일을 받는다.
	float fCubeScale = CPurpleCube::CUBESCALE;
...
HRESULT CPurpleCube::AddComponent(void)
{
	...
	// Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCEID::STATICMESH_CUBE_PURPLE);
...
void CPurpleCube::UpdateCollision(void)
{ 
	...
	{ // if
		CPurpleCube* pCube = NULL;

		switch(wEdge)
		{ // switch
		case PLANE_TOP:
			pCube = CPurpleCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.y = m_pInfo->m_vPos.y + CPurpleCube::CUBERADIUS;
			break;
		...
		case PLANE_LEFT:
			pCube = CPurpleCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.x = m_pInfo->m_vPos.x - CPurpleCube::CUBERADIUS;
			break;
		...
		case PLANE_FRONT:
			pCube = CPurpleCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.z = m_pInfo->m_vPos.z - CPurpleCube::CUBERADIUS;
			break;
...
void CPurpleCube::UpdateMovement(void)
{
...
bool CPurpleCube::IsIntersectedtoSelect(D3DXVECTOR3* pPos, WORD* pEdge)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
bool CPurpleCube::IsIntersectedtoMakeNavCell(D3DXVECTOR3* pPos)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
void CPurpleCube::ReleaseSelect(void)
{
	m_pSphere->ChangeColor(D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
	m_IsSelected = false; 
}

Engine::CTransform* CPurpleCube::GetCubeData(void)
{
	return m_pInfo;
}

void CPurpleCube::Release(void)
{
