...
COrangeCube* COrangeCube::Create(LPDIRECT3DDEVICE9 pDevice, Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
...
HRESULT COrangeCube::InitCube(Engine::CLayer* pLayer, const DWORD& dwCubeType)
{
	...
	/// 정해진 박스 스케일을 받는다.
	float fCubeScale = COrangeCube::CUBESCALE;
...
void COrangeCube::UpdateCollision(void)
{ 
	...
	{ // if
		COrangeCube* pCube = NULL;

		switch(wEdge)
		{ // switch
		case PLANE_TOP:
			pCube = COrangeCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.y = m_pInfo->m_vPos.y + COrangeCube::CUBERADIUS;
			break;
		...
		case PLANE_LEFT:
			pCube = COrangeCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.x = m_pInfo->m_vPos.x - COrangeCube::CUBERADIUS;
			break;
		...
		case PLANE_FRONT:
			pCube = COrangeCube::Create(m_pDevice, m_pLayer, m_dwCubeType);
			pCube->m_pInfo->m_vPos = m_pInfo->m_vPos;
			pCube->m_pInfo->m_vPos.z = m_pInfo->m_vPos.z - COrangeCube::CUBERADIUS;
			break;
...
void COrangeCube::UpdateMovement(void)
{
...
bool COrangeCube::IsIntersectedtoSelect(D3DXVECTOR3* pPos, WORD* pEdge)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
bool COrangeCube::IsIntersectedtoMakeNavCell(D3DXVECTOR3* pPos)
{
	Engine::RAY3 Ray3;
	m_pMouseObserver->GetData(CMouseSubject::DATAMESSAGE_RAY, (void*)&Ray3, CMouseSubject::DATATYPE_FLOAT, sizeof(Engine::RAY3));
...
void COrangeCube::ReleaseSelect(void)
{
	m_pSphere->ChangeColor(D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
	m_IsSelected = false; 
}

Engine::CTransform* COrangeCube::GetCubeData(void)
{
	return m_pInfo;
}
