...
Engine::CCubeColor* Engine::CCubeColor::Create(LPDIRECT3DDEVICE9 pDevice, const STANDARD_VERTEX3& StdVtx3, const DWORD& dwColor)
{
	CCubeColor*		pBuffer = new CCubeColor(pDevice);

	if(FAILED(pBuffer->CreateBuffer(StdVtx3, dwColor)))
		Engine::Safe_Delete(pBuffer);

	return pBuffer;
}

HRESULT Engine::CCubeColor::CreateBuffer(const STANDARD_VERTEX3& StdVtx3, const DWORD& dwColor)
{
...
Engine::CResource* Engine::CCubeColor::CloneResource(void)
{
